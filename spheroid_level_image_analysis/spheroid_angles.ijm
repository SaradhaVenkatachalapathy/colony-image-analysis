...
for (i=0; i<list1.length; i++) {
	...
	saveAs("Results",  Dir_res_ch2_int + baseName +".csv"); 	//save results
	run("Clear Results");	 									//clear results
	run("Close All");											//Close all iamges
	roiManager("reset");										//reset roi manager
}
...
for (i=0; i<list1.length; i++) {
	...
	saveAs("Results",  Dir_res_ch3_int + baseName +".csv"); 	//save results
	run("Clear Results");	 									//clear results
	run("Close All");											//Close all iamges
	roiManager("reset");										//reset roi manager
}
...
for (i=0; i<list1.length; i++) {
	...
	saveAs("Results",  Dir_res_ch4_int + baseName +".csv"); 	//save results
	run("Clear Results");	 									//clear results
	run("Close All");											//Close all iamges
	roiManager("reset");										//reset roi manager
}
