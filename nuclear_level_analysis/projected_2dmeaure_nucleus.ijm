...
run("Close All");
saveAs("Results",  dirres + "2D_nucleus_DNA.csv"); 
run("Clear Results");

if(nchannels>1){ 
	dir1= dirsa + "indivisual_nuclei"+ch2_name+ File.separator; 	// define the path to a new folder that will contain the second channel of the raw image cropped to represent each spheroid
	list1 = getFileList(dir1); // get the list of the files
	...
	run("Close All");
	saveAs("Results",  dirres + "2D_nucleus"+ch2_name+".csv"); 
	run("Clear Results");
}

if(nchannels>2){ 
	dir1= dirsa + "indivisual_nuclei"+ch3_name+ File.separator; 	// define the path to a new folder that will contain the third channel of the raw image cropped to represent each spheroid
	list1 = getFileList(dir1); // get the list of the files
	...
	run("Close All");
	saveAs("Results",  dirres + "2D_nucleus_"+ch3_name+".csv"); 
	run("Clear Results");
}

if(nchannels>3){ 
	dir1= dirsa + "indivisual_nuclei"+ch4_name+ File.separator; 	// define the path to a new folder that will contain the fourth channel of the raw image cropped to represent each spheroid
	list1 = getFileList(dir1); // get the list of the files
	...
	run("Close All");
	saveAs("Results",  dirres + "2D_nucleus_"+ch4_name+".csv"); 
	run("Clear Results");
}